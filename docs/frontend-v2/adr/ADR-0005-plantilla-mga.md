...
La decisión se implementó posteriormente, sin reutilizar la plantilla legacy
incompleta ni modificar el contenido aprobado en este ADR:

- `1ddc25f`: `templateId` `mga`, factoría con las siete secciones literales,
  validación fail-closed, persistencia y exportación DOCX.
...
Ninguno. Los incrementos `1ddc25f` y `199fa1e` implementaron y verificaron
conjuntamente las tres plantillas mínimas de RF-101.
